...
const FullLevelPage = () => {
  ...
  const { isGettingLevel, getLevelById, fullLevel } = useAdminStore();
  ...
  if (!fullLevel) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-900 p-6">
        <h2 className="text-2xl font-bold text-red-500">Level not found!</h2>
      </div>
    );
  }
  ...
  return (
    <div className="ml-64 flex-1 min-h-screen bg-gray-100 dark:bg-gray-900 p-10">
      ...
      <div className="max-w-4xl mx-auto mt-6 text-lg space-y-4">
        ...
        {fullLevel.lectures?.length > 0 ? (
          <ul className="space-y-4">
            {fullLevel.lectures.map((lecture, index) => (
              <li
                key={lecture._id}
        ...
        {/* Add Levels Button */}
        <button
          onClick={() => navigate(`/dashboard/add-lecture/${fullLevel._id}`)}
